...
public class App extends Application {
    ...
    TextField pid, btime, aTime,priority;
    ...
    public void set_inptable( int algo){
        ...
        TableColumn<InputData, Integer> col_pid = new TableColumn<>("ProdID");
        ...
        if( algo == 1 ) {
            // arrival time column
            TableColumn<InputData, Integer> col_aTime = new TableColumn<>("ARRIVAL TIME");
            col_aTime.setCellValueFactory(new PropertyValueFactory<>("atime"));
            col_aTime.setCellFactory(TextFieldTableCell.forTableColumn(new IntegerStringConverter()));
            col_aTime.setOnEditCommit(
                    new EventHandler<TableColumn.CellEditEvent<InputData, Integer>>() {
                        @Override
                        public void handle(TableColumn.CellEditEvent<InputData, Integer> edit) {
                            (edit.getTableView().getItems().get(
                                    edit.getTablePosition().getRow()
                            )).setAtime(edit.getNewValue());
                        }
                    }
            );
            table.getColumns().add(col_aTime);
        }
    ...
    public HBox set_updateinp(int algo) {
        ...
        if( algo == 1) {
            aTime = new TextField();
            aTime.setMaxWidth(100);
            aTime.setPromptText("ARRIVAL TIME");
            return new HBox(10, pid, btime, aTime, add, delete, submit);
        }
        ...
        if( !pid.getText().equals("") && !btime.getText().equals("") ) {
            ...
            if (algo == 1) {
                if( aTime.getText().equals("") ) {
                    MsgBox.show("ARRIVAL TIME IS EMPTY", "WARNING");
                    return;
                }
                data.setAtime(Integer.parseInt(aTime.getText()));
                aTime.clear();
            }
            ...
            System.out.println(data.getAtime());
    ...
    public void submit_data(int algo) {
        ...
        if( table.getItems().size() == Integer.parseInt(input_n.getText())) {
            ...
            for ( InputData data : rows ){
                ...
                if( algo == 1 ) {input_data.add(data.getAtime());}
    ...
    public void set_output(int n){
        ...
        System.out.println(load_outdata(output_table,n).toString());
        ...
        TableColumn<OutputData, Integer> out_col_atime = new TableColumn<>("ARRIVAL TIME");
        out_col_pid.setCellValueFactory(new PropertyValueFactory<>("atime"));

        TableColumn<OutputData, Integer> out_col_btime = new TableColumn<>("BURST TIMES");
        ...
        out_table.getColumns().add(out_col_atime);
    ...
    public ObservableList<OutputData> load_outdata(int[][] out_table, int n) {
        ...
        for( int i=0;i<n;i++ ) {
            data.add(new OutputData(out_table[i][0], out_table[i][1], out_table[i][2], out_table[i][3],out_table[i][4]));
        }
        return data;
    }
